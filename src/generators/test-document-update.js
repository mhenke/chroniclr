...
const testPRData = {
  ...
  deletions: 50,
  files: [
    { filename: 'src/auth/oauth.js', additions: 120, deletions: 0, status: 'added' },
    { filename: 'src/auth/user-profile.js', additions: 85, deletions: 0, status: 'added' },
    { filename: 'src/api/auth-routes.js', additions: 75, deletions: 10, status: 'modified' },
    { filename: 'tests/auth.test.js', additions: 50, deletions: 5, status: 'modified' },
    { filename: 'docs/API-Documentation.md', additions: 20, deletions: 35, status: 'modified' }
  ],
...
async function runTest() {
  ...
  try {
    ...
    const documentManager = await new DocumentUpdateManager(config).initialize();
    ...
    const { documentsToUpdate } = await documentManager.identifyDocumentsToUpdate(testPRData);
