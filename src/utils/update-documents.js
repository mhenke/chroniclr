...
async function updateFromPR(documentManager, prNumber, options) {
  ...
  if (options.dryRun) {
    ...
    const { documentsToUpdate, suggestedDocuments } =
      await documentManager.identifyDocumentsToUpdate(prData);
    ...
    const results = await documentManager.updateDocumentsFromPR(
      prData,
      options
    );
...
async function scanAndUpdate(documentManager, options) {
  ...
  console.log(
    `📊 Found ${documentsToUpdate.length} documents that need updating`
  );
