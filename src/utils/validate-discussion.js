...
function validateDiscussion(discussion) {
  ...
  if (!discussion.title || discussion.title.trim().length === 0) {
    errors.push('Discussion title is required');
  }
  
  if (!discussion.body || discussion.body.trim().length === 0) {
    errors.push('Discussion body is required');
  }
  
  if (!discussion.user?.login) {
...
function main() {
  try {
    ...
    if (!validation.isValid) {
      core.setFailed(`Discussion validation failed: ${validation.errors.join(', ')}`);
      process.exit(1);
    }
